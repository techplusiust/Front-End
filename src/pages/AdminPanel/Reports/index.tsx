...
const Reports: React.FC = () => {
  ...
  return (
    <div className="reports">
      <h1 className="reports-title">گزارش کاربران</h1>
      <input
        type="text"
        value={searchTerm}
        ...
        {filteredReports.length > 0 ? (
          filteredReports.map((report) => (
            <div key={report.id} className="report-card">
              ...
              <p><strong>تاریخ عضویت:</strong> {report.joinDate}</p>
              <p>
                <strong>تاریخ حذف پروفایل:</strong>{" "}
                {report.deleteDate ? report.deleteDate : "حذف نشده"}
              </p>
              <div>
                <strong>سه فعالیت اخیر در سایت:</strong>
          ...
          <p className="no-results">هیچ گزارشی یافت نشد.</p>
        )}
      </div>
    </div>
  );
